...
func (tx *Tx) init() {
	tx.Medias = NewMediasClient(tx.config)
	tx.Opinions = NewOpinionsClient(tx.config)
	tx.Userlogins = NewUserloginsClient(tx.config)
...
// applies a query, for example: Medias.QueryXXX(), the query will be executed
